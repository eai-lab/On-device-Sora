...
def bipartite_soft_matching_randframe(metric: torch.Tensor, 
    ...
    with torch.no_grad():
        ...
        randf = torch.randint(0, target_stride, torch.Size([1]), device=metric.device)
...
def bipartite_soft_matching_random2d(metric: torch.Tensor,
    ...
    with torch.no_grad():
        ...
        if no_rand:
            ...
            # torch.Size([36, 64, 1])
            rand_idx = torch.randint(sy*sx, size=(hsy, wsx, 1)).to(metric.device)
    ...
    def merge(x: torch.Tensor, mode="mean") -> torch.Tensor:
        src, dst = split(x)
        n, t1, c = src.shape
        # print("Merge", src.shape, dst.shape)
        unm = gather(src, dim=-2, index=unm_idx.expand(n, t1 - r, c))
        src = gather(src, dim=-2, index=src_idx.expand(n, r, c))
        dst = dst.scatter_reduce(-2, dst_idx.expand(n, r, c), src, reduce=mode)
        # print("Final", unm.shape, src.shape, dst.shape)
        return torch.cat([unm, dst], dim=1)
...
def compute_merge_default(x: torch.Tensor, original_h, original_w, ratio, use_rand = False) -> Tuple[Callable, ...]:
    ...
    use_rand = False if x.shape[0] % 2 == 1 else use_rand
    m, u = bipartite_soft_matching_random2d(x, w, h, 2, 2, r, no_rand=not use_rand)
    return m, u