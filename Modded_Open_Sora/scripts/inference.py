...
def main():
    ...
    # == opensora_category test==
    if not os.path.exists(f'/workspace/workspace/opensora-t5/Open-Sora/samples/{category}'):
        os.makedirs(f'/workspace/workspace/opensora-t5/Open-Sora/samples/{category}')
    ...
    print(f"vae.out_channels: {vae.out_channels}")
    print(f"text_encoder.output_dim: {text_encoder.output_dim}")
    print(f"text_encoder.model_max_length: {text_encoder.model_max_length}")
    ...
    # os.makedirs(f'{save_dir}/{cfg.lpl_setting}', exist_ok=True)
    # os.makedirs(f'{save_dir}/{cfg.lpl_setting}/{category}', exist_ok=True)
    # save_dir = f'{save_dir}/{cfg.lpl_setting}/{category}'
    ...
    for i in progress_wrap(range(0, len(prompts), batch_size)):
        ...
        for k in range(num_sample):
            ...
            def txt_to_tensor(file_path):
                    ...
                    return tensor
            if flag:
                test_output_path = "/workspace/opensora-t5/Open-Sora/configs/opensora-v1-2/inference/output.txt"
                test_output = txt_to_tensor(test_output_path).reshape(1,4,15,20,27).to(device, dtype)
                print("test_output shape: ", test_output.shape)
                test_sample = vae.decode(test_output.to(dtype), num_frames=num_frames)
                video_clips.append(test_sample)
            elif cfg.get("phone_output", False):
                # load text file
                # convert to tensor
                sample_tensor = txt_to_tensor('/workspace/opensora-t5/Open-Sora/scripts/phone_out.txt').reshape([1, 3, 51, 160, 216]).to(device, dtype)
                video_clips = [sample_tensor]
            else:
        ...
        print(f"Total runtime: {end_time - start_time:.6f} seconds")  # Print runtime
