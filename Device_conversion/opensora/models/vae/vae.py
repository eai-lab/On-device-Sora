...
class VideoAutoencoderPipeline(PreTrainedModel):
    ...
    def decode(self, z, num_frames=None):
        ...
        if self.micro_frame_size is None:
            ...
            x_z_list = []
            # before
            # z_bs = z[:, :, 0 : 0 + self.micro_z_frame_size]
            # after
            z_bs = z[:, :, 0 : 0 + self.micro_z_frame_size,:,:]
            
            x_z_bs = self.temporal_vae.decode(z_bs, num_frames=self.micro_frame_size)
            x_z_list.append(x_z_bs)
            # num_frames -= self.micro_frame_size

            z_bs = z[:, :, 5 : 5 + self.micro_z_frame_size,:,:]
            x_z_bs = self.temporal_vae.decode(z_bs, num_frames=self.micro_frame_size)
            x_z_list.append(x_z_bs)
            # num_frames -= self.micro_frame_size

            z_bs = z[:, :, 10 : 10 + self.micro_z_frame_size,:,:]
            x_z_bs = self.temporal_vae.decode(z_bs, num_frames=self.micro_frame_size)
            x_z_list.append(x_z_bs)
            # num_frames -= self.micro_frame_size
