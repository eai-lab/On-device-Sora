...
from .supplementary import BlockDiagonalMask, typical_sd_attention
import numpy as np
import torch
...
class PatchEmbed3D(nn.Module):
    ...
    def forward(self, x, pad_height, pad_width):
        """Forward function."""
        # padding
        _, _, D, H, W = x.size()
        # patch_size = [1,2,2]
        # if W % self.patch_size[2] != 0:
        #     pad_width = self.patch_size[2] - W % self.patch_size[2]
        #     x = torch.cat([x, torch.zeros(x.size(0), x.size(1), x.size(2), x.size(3), pad_width).to(x.device)], dim=4)
        # if H % self.patch_size[1] != 0:
        #     pad_height = self.patch_size[1] - H % self.patch_size[1]
        #     x = torch.cat([x, torch.zeros(x.size(0), x.size(1), x.size(2), pad_height, x.size(4)).to(x.device)], dim=3)
        # if D % self.patch_size[0] != 0:
        #     pad_depth = self.patch_size[0] - D % self.patch_size[0]
        #     x = torch.cat([x, torch.zeros(x.size(0), x.size(1),  pad_depth, x.size(3),  x.size(4)).to(x.device)], dim=2)
        x = torch.cat([x, torch.zeros(x.size(0), x.size(1), x.size(2), x.size(3), pad_width).to(x.device)], dim=4)
        x = torch.cat([x, torch.zeros(x.size(0), x.size(1), x.size(2), pad_height, x.size(4)).to(x.device)], dim=3)               
...
class MultiHeadCrossAttention(nn.Module):
    ...
    def forward(self, x, cond, mask=None):
        ...
        # if mask is not None:
            # fhma is 
            # attn_bias = BlockDiagonalMask.from_seqlens([N] * B, mask)
            # attn_bias = xformers.ops.fmha.BlockDiagonalMask.from_seqlens([N] * B, mask)
        # x = xformers.ops.memory_efficient_attention(q, k, v, p=self.attn_drop.p, attn_bias=attn_bias)
        x = typical_sd_attention(q, k, v, drop_probability=self.attn_drop.p, attn_bias=mask)
...
class PositionEmbedding2D(nn.Module):
    ...
    def forward(
        ...
        if base_size is not None:
            grid_h *= base_size / h
            grid_w *= base_size / w
        # Assuming grid_w and grid_h are your initial 1D tensors

        # Create repeating patterns using repeat
        new_grid_w = grid_h.repeat(w).view(w, h)
        new_grid_h = grid_w.view(-1,1).repeat(1, h)

        grid_h = new_grid_h.t().reshape(-1)
        grid_w = new_grid_w.t().reshape(-1)
        emb_h = self._get_sin_cos_emb(grid_h)
        emb_w = self._get_sin_cos_emb(grid_w)
        return torch.concat([emb_h, emb_w], dim=-1).unsqueeze(0).to(dtype)
