...
class BlockDiagonalMask(AttentionBias):
    ...
    def materialize(
        ...
        # Before: mask.fill_(torch.finfo(torch.float16).min)
        mask = torch.full_like(mask, -math.inf)
...
def typical_sd_attention(q, k, v, drop_probability=0.1, attn_bias=None):
    """
    Compute the attention.

    Args:
        q (torch.Tensor): Queries of shape (batch_size, num_heads, seq_length, d_k)
        k (torch.Tensor): Keys of shape (batch_size, num_heads, seq_length, d_k)
        v (torch.Tensor): Values of shape (batch_size, num_heads, seq_length, d_v)
        drop_probability (float): Dropout probability
        attn_bias (torch.Tensor or None): Bias to add to the attention scores of shape (batch_size, num_heads, seq_length, seq_length)
    ...
    # Add the bias
    # if attn_bias is not None:
        # attn_bias is in BlockDiagonalMask format. materialize is way too slow. 
        # attn_bias = attn_bias.materialize(attn.shape)
    ...
    attn = attn.softmax(-1)
    # Apply dropout
    attn = F.dropout(attn, drop_probability)
